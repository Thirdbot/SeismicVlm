...
def copy_test(nar, reader, scenes, use_reader):
    ...
    measured attribute across ALL classes (fault dip/throw + area for every area class — closure/salt/onlap):
    a value is "copied" if some evidence number is within ±2% of it (±0.5 absolute floor). Mirrors CHAIR's
    tolerance — NOT substring-inflated (a value's digits hiding inside a coordinate don't count) and NOT
    exact-match harsh (a faithful copy with minor precision drift still counts; only a genuine value change is
    a miss). use_reader=False = pure copy (GT in); True = pipeline (reader-capped)."""
    hit = tot = 0
